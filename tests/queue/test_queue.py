...
from review_app.queue.jobs import ping_job, render_print_job
...
def test_render_print_job_stub_raises() -> None:
    with pytest.raises(NotImplementedError, match="Phase 2/3"):
        render_print_job("spec-123")
...
def test_failed_job_lands_in_failed_queue(fake_redis: fakeredis.FakeRedis) -> None:
    ...
    job = enqueue(render_print_job, "spec-xyz", connection=fake_redis)
    q = get_queue(QUEUE_DEFAULT, connection=fake_redis)
    SimpleWorker([q], connection=fake_redis).work(burst=True)
    job.refresh()
    assert job.get_status() == JobStatus.FAILED
    assert job.exc_info is not None
    assert "NotImplementedError" in job.exc_info
